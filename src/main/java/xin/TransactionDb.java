...
final class TransactionDb {
    ...
    static TransactionImpl loadTransaction(Connection con, ResultSet rs) throws XinException.NotValidException {
        try {
            ...
            long amountTQT = rs.getLong("amount");
            ...
            TransactionImpl.BuilderImpl builder = new TransactionImpl.BuilderImpl(version, null,
                    amountTQT, feeTQT, deadline, transactionType.parseAttachment(buffer, version))
                    .timestamp(timestamp)
        ...
        try {
            ...
            for (TransactionImpl transaction : transactions) {
                try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO transaction (id, deadline, "
                    ...
                    pstmt.setLong(++i, transaction.getAmountTQT());
                    pstmt.setLong(++i, transaction.getFeeTQT());
