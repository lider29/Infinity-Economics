...
final class BlockImpl implements Block {
    ...
    private final long totalAmountTQT;
    private final long totalFeeTQT;
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountTQT, long totalFeeTQT, int payloadLength, byte[] payloadHash,
              byte[] generatorPublicKey, byte[] generationSignature, byte[] previousBlockHash, List<TransactionImpl>
                      transactions, byte[] blockATs,String secretPhrase) {
        this(version, timestamp, previousBlockId, totalAmountTQT, totalFeeTQT, payloadLength, payloadHash,
                generatorPublicKey, generationSignature, null, previousBlockHash, transactions,blockATs);
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountTQT, long totalFeeTQT, int payloadLength, byte[] payloadHash,
              ...
              List<TransactionImpl> transactions,byte[] blockATs) {
        this.version = version;
        this.timestamp = timestamp;
        this.previousBlockId = previousBlockId;
        this.totalAmountTQT = totalAmountTQT;
        this.totalFeeTQT = totalFeeTQT;
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountTQT, long totalFeeTQT, int payloadLength,
              ...
              List<TransactionImpl> blockTransactions,byte[] blockATs) {
        this(version, timestamp, previousBlockId, totalAmountTQT, totalFeeTQT, payloadLength, payloadHash,
                null, generationSignature, blockSignature, previousBlockHash, null,blockATs);
    ...
    @Override
    public long getTotalAmountTQT() {
        return totalAmountTQT;
    }

    @Override
    public long getTotalFeeTQT() {
        return totalFeeTQT;
    }
    ...
    public JSONObject getJSONObject() {
        ...
        json.put("totalAmountTQT", totalAmountTQT);
        json.put("totalFeeTQT", totalFeeTQT);
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws XinException.NotValidException {
        try {
            int version = ((Long) blockData.get("version")).intValue();
            int timestamp = ((Long) blockData.get("timestamp")).intValue();
            long previousBlock = Convert.parseUnsignedLong((String) blockData.get("previousBlock"));
            long totalAmountTQT = Convert.parseLong(blockData.get("totalAmountTQT"));
            long totalFeeTQT = Convert.parseLong(blockData.get("totalFeeTQT"));
            ...
            BlockImpl block = new BlockImpl(version, timestamp, previousBlock, totalAmountTQT, totalFeeTQT, payloadLength, payloadHash, generatorPublicKey,
                    generationSignature, blockSignature, previousBlockHash, blockTransactions,blockATs);
            if (!block.checkSignature()) {
                throw new XinException.NotValidException("Invalid block signature");
            }
            return block;
    ...
    byte[] bytes() {
        if (bytes == null) {
            ByteBuffer buffer = ByteBuffer.allocate(4 + 4 + 8 + 4 + (version < 3 ? (4 + 4) : (8 + 8)) + 4 + 32 + 32 +
                    (32 + 32) +(blockATs != null ? blockATs.length : 0)+ (blockSignature != null ? 64 : 0));
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountTQT / Constants.ONE_XIN));
                buffer.putInt((int) (totalFeeTQT / Constants.ONE_XIN));
            } else {
                buffer.putLong(totalAmountTQT);
                buffer.putLong(totalFeeTQT);
            }
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceTKN();
            ...
            return Generator.verifyHit(hit, BigInteger.valueOf(effectiveBalance), previousBlock, timestamp)
                    || (this.height < Constants.BAD_BLOCK_MAX_HEIGHT && Arrays.binarySearch(badBlocks, this.getId()) >= 0);
    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f token to forger at height %d", ((double) backFees[i]) / Constants.ONE_XIN, this.height - i - 1);
                previousGeneratorAccount.addToBalanceAndUnconfirmedBalanceTQT(LedgerEvent.BLOCK_GENERATED, getId(), backFees[i]);
                previousGeneratorAccount.addToForgedBalanceTQT(backFees[i]);
            }
        }
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f token at height %d", ((double) totalBackFees) / Constants.ONE_XIN, this.height);
        }
        generatorAccount.addToBalanceAndUnconfirmedBalanceTQT(LedgerEvent.BLOCK_GENERATED, getId(), totalFeeTQT - totalBackFees);
        generatorAccount.addToForgedBalanceTQT(totalFeeTQT - totalBackFees);
    }
