...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        BuilderImpl(byte version, byte[] senderPublicKey, long amountTQT, long feeTQT, short deadline,
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws XinException.NotValidException {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = Xin.getEpochTime();
            }
            if (!ecBlockSet) {
        ...
        @Override
        public TransactionImpl build() throws XinException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws XinException.NotValidException {

        this.timestamp = builder.timestamp;
        this.deadline = builder.deadline;
        this.senderPublicKey = builder.senderPublicKey;
        this.recipientId = builder.recipientId;
        this.amountTQT = builder.amountTQT;
        ...
        // Logger.logDebugMessage("Constant "+Constants.correctInvalidFees);
        // Logger.logDebugMessage("Fee TQT "+builder.feeTQT);
        // Logger.logDebugMessage("Signature "+builder.signature);
        //TODO:Check working when the fee is less than actual fee and auto correct disabled

        if (builder.feeTQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            //Logger.logErrorMessage("Correcting invalid fees");

            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Xin.getBlockchain().getHeight());
            long minFee = getMinimumFeeTQT(effectiveHeight);
            feeTQT = Math.max(minFee, builder.feeTQT);
        } else {
            // Logger.logErrorMessage("Not Correcting invalid fees");

            feeTQT = builder.feeTQT;
        }

        if (builder.signature != null && secretPhrase != null) {
            throw new XinException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && !Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new XinException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
    ...
    byte[] bytes() {
        if (bytes == null) {
            try {
                ...
                if (useTQT()) {
                    buffer.putLong(amountTQT);
                    buffer.putLong(feeTQT);
                    ...
                    buffer.putInt((int) (amountTQT / Constants.ONE_XIN));
                    buffer.putInt((int) (feeTQT / Constants.ONE_XIN));
                    if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws XinException.NotValidException {
        try {
            ...
            long amountTQT = buffer.getLong();
            long feeTQT = buffer.getLong();
            ...
            TransactionImpl.BuilderImpl builder = new BuilderImpl(version, senderPublicKey, amountTQT, feeTQT,
            ...
            if (buffer.hasRemaining()) {
                throw new XinException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (XinException.NotValidException | RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws XinException.NotValidException {
    ...
    public JSONObject getJSONObject() {
        ...
        json.put("amountTQT", amountTQT);
        json.put("feeTQT", feeTQT);
        if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws XinException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new XinException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws XinException.NotValidException {
        try {
            ...
            long amountTQT = Convert.parseLong(transactionData.get("amountTQT"));
            long feeTQT = Convert.parseLong(transactionData.get("feeTQT"));
            ...
            if (transactionType == null) {
                throw new XinException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }

            TransactionImpl.BuilderImpl builder = new BuilderImpl(version, senderPublicKey,
                    amountTQT, feeTQT, deadline,
    ...
    private boolean checkSignature() {
        if (!hasValidSignature) {
            hasValidSignature = signature != null && Crypto.verify(signature, zeroSignature(getBytes()), getSenderPublicKey(), useTQT());
        }
    ...
    private int signatureOffset() {
        return 1 + 1 + 4 + 2 + 32 + 8 + (useTQT() ? 8 + 8 + 32 : 4 + 4 + 8);
    }

    private boolean useTQT() {
        // return true;
        return this.height > Constants.TQT_BLOCK;
    }
    ...
    @Override
    public void validate() throws XinException.ValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeTQT != 0) : (deadline < 1 || feeTQT <= 0)
                || feeTQT > Constants.MAX_BALANCE_TQT
                || amountTQT < 0
                || amountTQT > Constants.MAX_BALANCE_TQT
                || type == null) {
            throw new XinException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
                    + ", deadline: " + deadline + ", fee: " + feeTQT + ", amount: " + amountTQT);
        }

        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new XinException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new XinException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (!type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountTQT() != 0) {
                throw new XinException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new XinException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (!appendage.verifyVersion(this.version)) {
                throw new XinException.NotValidException("Invalid attachment version " + appendage.getVersion()
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new XinException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }

        if (!validatingAtFinish) {
            long minimumFeeTQT = getMinimumFeeTQT(Xin.getBlockchain().getHeight());
            if (feeTQT < minimumFeeTQT) {

                throw new RuntimeException(String.format("Transaction fee %f token less than minimum fee %f token at height %d",
                        ((double) feeTQT) / Constants.ONE_XIN, ((double) minimumFeeTQT) / Constants.ONE_XIN, Xin.getBlockchain().getHeight()));
    ...
    void apply() {
        ...
        if (referencedTransactionFullHash != null
            ...
            senderAccount.addToUnconfirmedBalanceTQT(getType().getLedgerEvent(), getId(),
                    0, Constants.UNCONFIRMED_POOL_DEPOSIT_TQT);
        }
        if (attachmentIsPhased()) {
            senderAccount.addToBalanceTQT(getType().getLedgerEvent(), getId(), 0, -feeTQT);
        }
        for (Appendix.AbstractAppendix appendage : appendages) {
    ...
    private long getMinimumFeeTQT(int blockchainHeight) {
        long totalFee = 0;
        for (Appendix.AbstractAppendix appendage : appendages) {
            appendage.loadPrunable(this);
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_XIN);
        }
        return totalFee;
    }
