...
import xin.proxies.ripple.ProxyRippleClient;
...
public class ProxyRipple extends APIServlet.APIRequestHandler {

    static final ProxyRipple instance = new ProxyRipple();

    private static final String COMMAND_GET_ADDRESS = "xGetAddress";

    private static final String RIPPLE_AVAILABLE = Xin.getStringProperty("xin.proxy.xrp.enable");

    private ProxyRipple() {
        super(new APITag[]{APITag.PROXIES});
        btcRippleClient = new ProxyRippleClient();
    }
    ...
    protected JSONStreamAware processRequest(HttpServletRequest request) throws XinException {
        ...
        switch (command) {
            case COMMAND_GET_ADDRESS:
                jsonObject = btcRippleClient.getRippleBalance(requestParams);
                break;
            default:
        ...
        jsonObject.put("rippleAvailable", true);
        return jsonObject;
    }

    private boolean isProxyAvailable() {
        return Xin.getBooleanProperty("xin.proxy.xrp.enable"); // StringUtils.equalsIgnoreCase("true", RIPPLE_AVAILABLE);
    }
