...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId, long amountTQT)
            throws XinException {
        return createTransaction(req, senderAccount, recipientId, amountTQT, Attachment.ORDINARY_PAYMENT);
    }
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Xin.getBlockchain().getHeight() + 1,
                Xin.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    // delimiter based stringsplitting
    final PhasingParams parsePhasingParams(HttpServletRequest req, String parameterPrefix, String delimiter) throws ParameterException {
        ...
        String delimiterSeparatedAccounts = Convert.emptyToNull(req.getParameter(parameterPrefix+"Whitelisted"));
        String[] whitelistValues = delimiterSeparatedAccounts.split(delimiter);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
        ...
        long feeTQT = ParameterParser.getFeeTQT(req);
        ...
        try {
            Transaction.Builder builder = Xin.newTransactionBuilder(publicKey, amountTQT, feeTQT,
            ...
            try {
                if (Math.addExact(amountTQT, transaction.getFeeTQT()) > senderAccount.getUnconfirmedBalanceTQT()) {
            ...
            } catch (XinException.NotYetEncryptedException ignore) {
            }
            ...
            if (broadcast) {
                Xin.getTransactionProcessor().broadcast(transaction);
                response.put("broadcasted", true);
            } else {
                transaction.validate();
                response.put("broadcasted", false);
            }
        } catch (XinException.NotYetEnabledException e) {
