...
public interface Appendix {
    ...
    interface Prunable {
        ...
        default boolean shouldLoadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            return Xin.getEpochTime() - transaction.getTimestamp() <
    ...
    abstract class AbstractAppendix implements Appendix {
        ...
        abstract void validate(Transaction transaction) throws XinException.ValidationException;

        void validateAtFinish(Transaction transaction) throws XinException.ValidationException {
    ...
    class Message extends AbstractAppendix {
        ...
        private static final Fee MESSAGE_FEE = new Fee.SizeBasedFee(Constants.FEE_MESSAGE_INITIAL_CONSTANT, Constants.MESSAGE_FEE_PER_UNIT, Constants.SIZE_MESSAGE_UNIT) {
            @Override
        ...
        Message(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
            ...
            if (messageLength > 1000) {
                throw new XinException.NotValidException("Invalid arbitrary message length: " + messageLength);
            }
            this.message = new byte[messageLength];
            buffer.get(this.message);
            if (isText && !Arrays.equals(message, Convert.toBytes(Convert.toString(message)))) {
                throw new XinException.NotValidException("Message is not UTF-8 text");
            }
        }
        ...
        @Override
        void validate(Transaction transaction) throws XinException.ValidationException {
            if (Xin.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new XinException.NotValidException("Invalid arbitrary message length: " + message.length);
            }
        }
    ...
    class PrunablePlainMessage extends Appendix.AbstractAppendix implements Prunable {
        ...
        // private static final Fee PRUNABLE_MESSAGE_FEE = new Fee.SizeBasedFee( Constants.ONE_XIN / 10 ) {

        private static final Fee PRUNABLE_MESSAGE_FEE = new Fee.SizeBasedFee( Constants.FEE_PRUNABLE_PLAIN_MESSAGE ) {
            @Override
        ...
        void validate(Transaction transaction) throws XinException.ValidationException {
            if (transaction.getMessage() != null) {
                throw new XinException.NotValidException("Cannot have both message and prunable message attachments");
            }
            byte[] msg = getMessage();
            if (msg != null && msg.length > Constants.MAX_PRUNABLE_MESSAGE_LENGTH) {
                throw new XinException.NotValidException("Invalid prunable message length: " + msg.length);
            }
            if (msg == null && Xin.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new XinException.NotCurrentlyValidException("Message has been pruned prematurely");
            }
        }

        @Override
        void apply(Transaction transaction, Account senderAccount, Account recipientAccount) {
            if (Xin.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
    ...
    abstract class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private static final Fee ENCRYPTED_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.FEE_ENCRYPTED_MESSAGE_INITIAL_CONSTANT, Constants.ENCRYPTED_MESSAGE_FEE_PER_UNIT, Constants.SIZE_ENCRYPTED_MESSAGE_UNIT) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendage) {
        ...
        private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
            super(buffer, transactionVersion);
        ...
        @Override
        void validate(Transaction transaction) throws XinException.ValidationException {
            if (Xin.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && getEncryptedDataLength() > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
                throw new XinException.NotValidException("Max encrypted message length exceeded");
            }
            if (encryptedData != null) {
                if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
                    ...
                    throw new XinException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
                }
            }
            if ((getVersion() != 2 && !isCompressed) || (getVersion() == 2 && isCompressed)) {
                throw new XinException.NotValidException("Version mismatch - version " + getVersion() + ", isCompressed " + isCompressed);
            }
        }
    ...
    class PrunableEncryptedMessage extends AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_ENCRYPTED_DATA_FEE = new Fee.SizeBasedFee( Constants.FEE_PRUNABLE_ENCRYPTED_MESSAGE ) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendix) {
                return appendix.getFullSize();
            }
        ...
        void validate(Transaction transaction) throws XinException.ValidationException {
            if (transaction.getEncryptedMessage() != null) {
                throw new XinException.NotValidException("Cannot have both encrypted and prunable encrypted message attachments");
            }
            EncryptedData ed = getEncryptedData();
            if (ed == null && Xin.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new XinException.NotCurrentlyValidException("Encrypted message has been pruned prematurely");
            }
            if (ed != null) {
                if (ed.getData().length > Constants.MAX_PRUNABLE_ENCRYPTED_MESSAGE_LENGTH) {
                    throw new XinException.NotValidException(String.format("Message length %d exceeds max prunable encrypted message length %d",
                ...
                if ((ed.getNonce().length != 32 && ed.getData().length > 0)
                    ...
                    throw new XinException.NotValidException("Invalid nonce length " + ed.getNonce().length);
                }
            }
            if (transaction.getRecipientId() == 0) {
                throw new XinException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
            }
        }
    ...
    final class UnencryptedPrunableEncryptedMessage extends PrunableEncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new XinException.NotYetEncryptedException("Prunable encrypted message not yet encrypted");
            }
    ...
    class EncryptedMessage extends AbstractEncryptedMessage {
        ...
        EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class UnencryptedEncryptedMessage extends EncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new XinException.NotYetEncryptedException("Message not yet encrypted");
            }
            super.putMyBytes(buffer);
        }
    ...
    class EncryptToSelfMessage extends AbstractEncryptedMessage {
        ...
        EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        void validate(Transaction transaction) throws XinException.ValidationException {
            if (transaction.getRecipientId() == 0) {
                throw new XinException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
            }
            if (!Crypto.isCanonicalPublicKey(publicKey)) {
                throw new XinException.NotValidException("Invalid recipient public key: " + Convert.toHexString(publicKey));
            }
            long recipientId = transaction.getRecipientId();
            if (Account.getId(this.publicKey) != recipientId) {
                throw new XinException.NotValidException("Announced public key does not match recipient accountId");
            }
            ...
            if (recipientPublicKey != null && !Arrays.equals(publicKey, recipientPublicKey)) {
                throw new XinException.NotCurrentlyValidException("A different public key for this account has already been announced");
            }
        }
    ...
    final class Phasing extends AbstractAppendix {
        ...
        private static final Fee PHASING_FEE = (transaction, appendage) -> {
            ...
            if (!phasing.params.getVoteWeighting().isBalanceIndependent()) {
                fee += Constants.FEE_PHASING_VOTE_UNIT;
            } else {
                fee += Constants.FEE_PHASING_INITIAL_CONSTANT;
            }
            if (phasing.hashedSecret.length > 0) {
                fee += (1 + (phasing.hashedSecret.length - 1) / Constants.SIZE_PHASING_MESSAGE_UNIT) * Constants.FEE_PHASING_INITIAL_CONSTANT;
            }
            fee += Constants.FEE_PHASING_INITIAL_CONSTANT * phasing.linkedFullHashes.length;
            return fee;
        };
        ...
        void validate(Transaction transaction) throws XinException.ValidationException {
            ...
            if (params.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.TRANSACTION) {
                if (linkedFullHashes.length == 0 || linkedFullHashes.length > Constants.MAX_PHASING_LINKED_TRANSACTIONS) {
                    throw new XinException.NotValidException("Invalid number of linkedFullHashes " + linkedFullHashes.length);
                }
                Set<Long> linkedTransactionIds = new HashSet<>(linkedFullHashes.length);
                for (byte[] hash : linkedFullHashes) {
                    if (Convert.emptyToNull(hash) == null || hash.length != 32) {
                        throw new XinException.NotValidException("Invalid linkedFullHash " + Convert.toHexString(hash));
                    }
                    if (Xin.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK) {
                        if (!linkedTransactionIds.add(Convert.fullHashToId(hash))) {
                            throw new XinException.NotValidException("Duplicate linked transaction ids");
                        }
                    }
                    ...
                    if (linkedTransaction != null) {
                        if (transaction.getTimestamp() - linkedTransaction.getTimestamp() > Constants.MAX_REFERENCED_TRANSACTION_TIMESPAN) {
                            throw new XinException.NotValidException("Linked transaction cannot be more than 60 days older than the phased transaction");
                        }
                        if (linkedTransaction.getPhasing() != null) {
                            throw new XinException.NotCurrentlyValidException("Cannot link to an already existing phased transaction");
                        }
                    }
                }
                if (params.getQuorum() > linkedFullHashes.length) {
                    throw new XinException.NotValidException("Quorum of " + params.getQuorum() + " cannot be achieved in by-transaction voting with "
                            + linkedFullHashes.length + " linked full hashes only");
                }
            } else {
                if (linkedFullHashes.length != 0) {
                    throw new XinException.NotValidException("LinkedFullHashes can only be used with VotingModel.TRANSACTION");
                }
            }

            if (params.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.HASH) {
                if (params.getQuorum() != 1) {
                    throw new XinException.NotValidException("Quorum must be 1 for by-hash voting");
                }
                if (hashedSecret.length == 0 || hashedSecret.length > Byte.MAX_VALUE) {
                    throw new XinException.NotValidException("Invalid hashedSecret " + Convert.toHexString(hashedSecret));
                }
                if (PhasingPoll.getHashFunction(algorithm) == null) {
                    throw new XinException.NotValidException("Invalid hashedSecretAlgorithm " + algorithm);
                }
            } else {
                if (hashedSecret.length != 0) {
                    throw new XinException.NotValidException("HashedSecret can only be used with VotingModel.HASH");
                }
                if (algorithm != 0) {
                    throw new XinException.NotValidException("HashedSecretAlgorithm can only be used with VotingModel.HASH");
                }
            }
            ...
            if (finishHeight <= currentHeight + (params.getVoteWeighting().acceptsVotes() ? 2 : 1)
                ...
                throw new XinException.NotCurrentlyValidException("Invalid finish height " + finishHeight);
            }
        }
        ...
        void reject(TransactionImpl transaction) {
            ...
            senderAccount.addToUnconfirmedBalanceTQT(LedgerEvent.REJECT_PHASED_TRANSACTION, transaction.getId(),
                    transaction.getAmountTQT());
        ...
        void countVotes(TransactionImpl transaction) {
            if (Xin.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && PhasingPoll.getResult(transaction.getId()) != null) {
                return;
            }
            ...
            if (result >= poll.getQuorum()) {
                if (!transaction.attachmentIsDuplicate(duplicates, false)) {
                    try {
                        release(transaction);
                        poll.finish(result);
                        Logger.logDebugMessage("Early finish of transaction " + transaction.getStringId() + " at height " + Xin.getBlockchain().getHeight());
                    } catch (RuntimeException e) {
                        Logger.logErrorMessage("Failed to release phased transaction " + transaction.getJSONObject().toJSONString(), e);
                    }
                } else {
                    Logger.logDebugMessage("At height " + Xin.getBlockchain().getHeight() + " phased transaction " + transaction.getStringId()
