...
public class AT_API_Platform_Impl extends AT_API_Impl {
	...
	public void put_Last_Block_Hash_In_A( AT_Machine_State state ) {
		...
		b.put( Xin.getBlockchain().getBlockAtHeight(state.getHeight() - 1).getBlockHash() );
	...
	public long get_Type_for_Tx_in_A( AT_Machine_State state ) {
		...
		Transaction tx = Xin.getBlockchain().getTransaction( txid );
	...
	public long get_Amount_for_Tx_in_A( AT_Machine_State state ) {
		...
		if ( tx != null )
		{
			...
			// if( (tx.getMessage() == null || state.getHeight() >= Constants.AT_FIX_BLOCK_2) && state
			// 		.minActivationAmount() <= tx.getAmountTQT() )
			//
			// {
			// 	amount = tx.getAmountTQT() - state.minActivationAmount();
			// }
			// else
			// {
			// 	amount = 0;
	...
	public long get_Random_Id_for_Tx_in_A( AT_Machine_State state ) {
		...
		if ( tx !=null )
		{
			...
			bf.put(Xin.getBlockchain().getBlockAtHeight(blockHeight - 1).getGenerationSignature());
			bf.putLong( tx.getId() );
	...
	public void put_Last_Block_Generation_Signature_In_A( AT_Machine_State state ) {
		...
		b.put( Xin.getBlockchain().getBlockAtHeight(state.getHeight() - 1).getGenerationSignature() );
