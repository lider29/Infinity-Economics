...
public abstract class TransactionType {
    ...
    abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException;

    abstract Attachment.AbstractAttachment parseAttachment(JSONObject attachmentData) throws XinException.NotValidException;
    ...
    public static abstract class Payment extends TransactionType {
        ...
        public static final TransactionType ORDINARY = new Payment() {
            ...
            @Override
            Attachment.EmptyAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return Attachment.ORDINARY_PAYMENT;
            }

            @Override
            Attachment.EmptyAttachment parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
                return Attachment.ORDINARY_PAYMENT;
            }
    ...
    public static abstract class Messaging extends TransactionType {
        ...
        public final static TransactionType ARBITRARY_MESSAGE = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (transaction.getAmountTQT() != 0) {
                    throw new XinException.NotValidException("Invalid arbitrary message: " + attachment.getJSONObject());
                }


                if (transaction.getRecipientId() == Genesis.CREATOR_ID && Xin.getBlockchain().getHeight() > Constants.NON_GENESIS_BLOCK_START_HEIGHT) {
        ...
        public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
            ...
            private final Fee ALIAS_FEE = new Fee.SizeBasedFee(Constants.FEE_ALIAS_INITIAL_CONSTANT, Constants.ALIAS_FEE_PER_UNIT, Constants.SIZE_ALIAS_UNIT) {
                @Override
            ...
            @Override
            Attachment.MessagingAliasAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingAliasAssignment(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasAssignment parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (alias != null && alias.getAccountId() != transaction.getSenderId()) {
                    throw new XinException.NotCurrentlyValidException("Alias already owned by another account: " + normalizedAlias);
                }
            }
        ...
        public static final TransactionType ALIAS_SELL = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingAliasSell(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasSell parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
        ...
        public static final TransactionType ALIAS_BUY = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingAliasBuy(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasBuy parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
        ...
        public static final TransactionType ALIAS_DELETE = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasDelete parseAttachment(final ByteBuffer buffer, final byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingAliasDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasDelete parseAttachment(final JSONObject attachmentData) throws XinException.NotValidException {
        ...
        public final static TransactionType POLL_CREATION = new Messaging() {
            ...
            private final Fee POLL_OPTIONS_FEE = new Fee.SizeBasedFee(Constants.FEE_POLL_OPTIONS_INITIAL_CONSTANT, Constants.POLL_OPTIONS_FEE_PER_UNIT, Constants.SIZE_POLL_OPTIONS_UNIT) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            private final Fee POLL_SIZE_FEE = new Fee.SizeBasedFee(Constants.FEE_POLL_SIZE_INITIAL_CONSTANT, Constants.POLL_SIZE_FEE_PER_UNIT, Constants.SIZE_POLL_SIZE_UNIT) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.MessagingPollCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingPollCreation(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingPollCreation parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (attachment.getMinNumberOfOptions() < 1
                    ...
                    throw new XinException.NotValidException("Invalid min number of options: " + attachment.getJSONObject());
                }
                ...
                if (attachment.getMaxNumberOfOptions() < 1
                    ...
                    throw new XinException.NotValidException("Invalid max number of options: " + attachment.getJSONObject());
                }

                for (int i = 0; i < optionsCount; i++) {
                    if (attachment.getPollOptions()[i].length() > Constants.MAX_POLL_OPTION_LENGTH
                            || attachment.getPollOptions()[i].isEmpty()) {
                        throw new XinException.NotValidException("Invalid poll options length: " + attachment.getJSONObject());
                    }
                }
                ...
                if (attachment.getFinishHeight() <= attachment.getFinishValidationHeight(transaction) + 1
                    ...
                    throw new XinException.NotCurrentlyValidException("Invalid finishing height" + attachment.getJSONObject());
                }

                if (!attachment.getVoteWeighting().acceptsVotes() || attachment.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.HASH) {
                    throw new XinException.NotValidException("VotingModel " + attachment.getVoteWeighting().getVotingModel() + " not valid for regular polls");
                }
        ...
        public final static TransactionType VOTE_CASTING = new Messaging() {
            ...
            @Override
            Attachment.MessagingVoteCasting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingVoteCasting(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingVoteCasting parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (attachment.getPollId() == 0 || attachment.getPollVote() == null
                    ...
                    throw new XinException.NotValidException("Invalid vote casting attachment: " + attachment.getJSONObject());
                }
                ...
                if (poll == null) {
                    throw new XinException.NotCurrentlyValidException("Invalid poll: " + Long.toUnsignedString(attachment.getPollId()));
                }
                ...
                if (poll.getFinishHeight() <= attachment.getFinishValidationHeight(transaction)) {
                    throw new XinException.NotCurrentlyValidException("Voting for this poll finishes at " + poll.getFinishHeight());
                }
                ...
                for (byte vote : votes) {
                    if (vote != Constants.NO_VOTE_VALUE && (vote < poll.getMinRangeValue() || vote > poll.getMaxRangeValue())) {
                        throw new XinException.NotValidException(String.format("Invalid vote %d, vote must be between %d and %d",
                ...
                if (positiveCount < poll.getMinNumberOfOptions() || positiveCount > poll.getMaxNumberOfOptions()) {
                    throw new XinException.NotValidException(String.format("Invalid num of choices %d, number of choices must be between %d and %d",
        ...
        public static final TransactionType PHASING_VOTE_CASTING = new Messaging() {
            ...
            @Override
            Attachment.MessagingPhasingVoteCasting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingPhasingVoteCasting(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingPhasingVoteCasting parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (hashes.size() > Constants.MAX_PHASING_VOTE_TRANSACTIONS) {
                    throw new XinException.NotValidException("No more than " + Constants.MAX_PHASING_VOTE_TRANSACTIONS + " votes allowed for two-phased multi-voting");
                }
                ...
                for (byte[] hash : hashes) {
                    ...
                    if (phasedTransactionId == 0) {
                        throw new XinException.NotValidException("Invalid phased transactionFullHash " + Convert.toHexString(hash));
                    }

                    PhasingPoll poll = PhasingPoll.getPoll(phasedTransactionId);
                    if (poll == null) {
                        throw new XinException.NotCurrentlyValidException("Invalid phased transaction " + Long.toUnsignedString(phasedTransactionId)
                                + ", or phasing is finished");
                    }
                    if (!poll.getVoteWeighting().acceptsVotes()) {
                        throw new XinException.NotValidException("This phased transaction does not require or accept voting");
                    }
                    ...
                    if (revealedSecret.length > 0) {
                        if (poll.getVoteWeighting().getVotingModel() != VoteWeighting.VotingModel.HASH) {
                            throw new XinException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " does not accept by-hash voting");
                        }
                        if (hashedSecret != null && !Arrays.equals(poll.getHashedSecret(), hashedSecret)) {
                            throw new XinException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " is using a different hashedSecret");
                        }
                        if (algorithm != 0 && algorithm != poll.getAlgorithm()) {
                            throw new XinException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " is using a different hashedSecretAlgorithm");
                        }
                        if (hashedSecret == null && !poll.verifySecret(revealedSecret)) {
                            throw new XinException.NotValidException("Revealed secret does not match phased transaction hashed secret");
                        }
                        ...
                        throw new XinException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " requires revealed secret for approval");
                    }
                    if (!Arrays.equals(poll.getFullHash(), hash)) {
                        throw new XinException.NotCurrentlyValidException("Phased transaction hash does not match hash in voting transaction");
                    }
                    if (poll.getFinishHeight() <= attachment.getFinishValidationHeight(transaction) + 1) {
                        throw new XinException.NotCurrentlyValidException(String.format("Phased transaction finishes at height %d which is not after approval transaction height %d",
        ...
        public static final TransactionType HUB_ANNOUNCEMENT = new Messaging() {
            ...
            @Override
            Attachment.MessagingHubAnnouncement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingHubAnnouncement(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingHubAnnouncement parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (Xin.getBlockchain().getHeight() < Constants.HUB_ANNOUNCEMENT_BLOCK) {
                    throw new XinException.NotYetEnabledException("Hub terminal announcement not yet enabled at height " + Xin.getBlockchain().getHeight());
                }
                ...
                if (attachment.getMinFeePerByteTQT() < 0 || attachment.getMinFeePerByteTQT() > Constants.MAX_BALANCE_TQT
                    ...
                    throw new XinException.NotValidException("Invalid hub terminal announcement: " + attachment.getJSONObject());
                }
        ...
        public static final Messaging ACCOUNT_INFO = new Messaging() {
            ...
            private final Fee ACCOUNT_INFO_FEE = new Fee.SizeBasedFee(Constants.FEE_ACCOUNT_INFO_INITIAL_CONSTANT, Constants.ACCOUNT_INFO_FEE_PER_UNIT, Constants.SIZE_ACCOUNT_INFO_UNIT) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.MessagingAccountInfo parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingAccountInfo(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountInfo parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (attachment.getName().length() > Constants.MAX_ACCOUNT_NAME_LENGTH
                    ...
                    throw new XinException.NotValidException("Invalid account info issuance: " + attachment.getJSONObject());
                }
            }
        ...
        public static final Messaging ACCOUNT_PROPERTY = new Messaging() {
            ...
            private final Fee ACCOUNT_PROPERTY_FEE = new Fee.SizeBasedFee(Constants.FEE_ACCOUNT_PROPERTY_INITIAL_CONSTANT, Constants.ACCOUNT_PROPERTY_FEE_PER_UNIT, Constants.SIZE_ACCOUNT_PROPERTY_UNIT) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.MessagingAccountProperty parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingAccountProperty(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountProperty parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (transaction.getAmountTQT() != 0) {
                    throw new XinException.NotValidException("Account property transaction cannot be used to send token");
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
        ...
        public static final Messaging ACCOUNT_PROPERTY_DELETE = new Messaging() {
            ...
            @Override
            Attachment.MessagingAccountPropertyDelete parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.MessagingAccountPropertyDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountPropertyDelete parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (accountProperty == null) {
                    throw new XinException.NotCurrentlyValidException("No such property " + Long.toUnsignedString(attachment.getPropertyId()));
                }
                if (accountProperty.getRecipientId() != transaction.getSenderId() && accountProperty.getSetterId() != transaction.getSenderId()) {
                    throw new XinException.NotValidException("Account " + Long.toUnsignedString(transaction.getSenderId())
                ...
                if (accountProperty.getRecipientId() != transaction.getRecipientId()) {
                    throw new XinException.NotValidException("Account property " + Long.toUnsignedString(attachment.getPropertyId())
    ...
    public static abstract class ColoredCoins extends TransactionType {
        ...
        public static final TransactionType ASSET_ISSUANCE = new ColoredCoins() {
            ...
            private final Fee SINGLETON_ASSET_FEE = new Fee.SizeBasedFee(Constants.FEE_ASSET_INITIAL_CONSTANT, Constants.ASSET_FEE_PER_UNIT, Constants.SIZE_ASSET_UNIT) {
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.ColoredCoinsAssetIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.ColoredCoinsAssetIssuance(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetIssuance parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if(Asset.isAssetNameTaken(normalizedName) && !Constants.ALLOW_DUPLICATE_ASSE_NAME){
                    throw new XinException.NotValidException("Invalid asset name : "+normalizedName+". Name already " + "taken");
                }
            }
        ...
        public static final TransactionType ASSET_TRANSFER = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.ColoredCoinsAssetTransfer(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetTransfer parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (transaction.getAmountTQT() != 0
                    ...
                    throw new XinException.NotValidException("Invalid asset transfer amount or comment: " + attachment.getJSONObject());
                }
                ...
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new XinException.NotValidException("Invalid asset transfer asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new XinException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
                }
            }
        ...
        public static final TransactionType ASSET_DELETE = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetDelete parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.ColoredCoinsAssetDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetDelete parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                Asset asset = Asset.getAsset(attachment.getAssetId());
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new XinException.NotValidException("Invalid asset delete asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new XinException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
        ...
        public static final TransactionType ASSET_COMPLETE_DELETE = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetCompleteDelete parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException
                    .NotValidException {
                return new Attachment.ColoredCoinsAssetCompleteDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetCompleteDelete parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            @Override
            void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
                Attachment.ColoredCoinsAssetCompleteDelete attachment = (Attachment.ColoredCoinsAssetCompleteDelete) transaction.getAttachment();
        ...
        abstract static class ColoredCoinsOrderPlacement extends ColoredCoins {
            ...
            final void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (attachment.getPriceTQT() <= 0 || attachment.getPriceTQT() > Constants.MAX_BALANCE_TQT
                        || attachment.getAssetId() == 0) {
                    throw new XinException.NotValidException("Invalid asset order placement: " + attachment.getJSONObject());
                }
                Asset asset = Asset.getAsset(attachment.getAssetId());
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new XinException.NotValidException("Invalid asset order placement asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new XinException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
        ...
        public static final TransactionType ASK_ORDER_PLACEMENT = new ColoredCoins.ColoredCoinsOrderPlacement() {
            ...
            @Override
            Attachment.ColoredCoinsAskOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.ColoredCoinsAskOrderPlacement(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAskOrderPlacement parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
        ...
        public final static TransactionType BID_ORDER_PLACEMENT = new ColoredCoins.ColoredCoinsOrderPlacement() {
            ...
            @Override
            Attachment.ColoredCoinsBidOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.ColoredCoinsBidOrderPlacement(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsBidOrderPlacement parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
                ...
                if (senderAccount.getUnconfirmedBalanceTQT() >= Math.multiplyExact(attachment.getQuantityQNT(), attachment.getPriceTQT())) {
                    senderAccount.addToUnconfirmedBalanceTQT(getLedgerEvent(), transaction.getId(),
        ...
        public static final TransactionType ASK_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
            ...
            @Override
            Attachment.ColoredCoinsAskOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.ColoredCoinsAskOrderCancellation(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAskOrderCancellation parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (ask == null) {
                    throw new XinException.NotCurrentlyValidException("Invalid ask order: " + Long.toUnsignedString(attachment.getOrderId()));
                }
                if (ask.getAccountId() != transaction.getSenderId()) {
                    throw new XinException.NotValidException("Order " + Long.toUnsignedString(attachment.getOrderId()) + " was created by account "
        ...
        public static final TransactionType BID_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
            ...
            @Override
            Attachment.ColoredCoinsBidOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.ColoredCoinsBidOrderCancellation(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsBidOrderCancellation parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (bid == null) {
                    throw new XinException.NotCurrentlyValidException("Invalid bid order: " + Long.toUnsignedString(attachment.getOrderId()));
                }
        ...
        public static final TransactionType DIVIDEND_PAYMENT = new ColoredCoins() {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (attachment.getHeight() > Xin.getBlockchain().getHeight()) {
                    throw new XinException.NotCurrentlyValidException("Invalid dividend payment height: " + attachment.getHeight()
                ...
                if (asset.getAccountId() != transaction.getSenderId() || attachment.getAmountTQTPerQNT() <= 0) {
                    throw new XinException.NotValidException("Invalid dividend payment sender or amount " + attachment.getJSONObject());
                }
                ...
                if (Xin.getBlockchain().getHeight() > Constants.DIVIDEND_THROTTLE_BLOCK) {
                    ...
                    if (lastDividend != null && lastDividend.getHeight() > Xin.getBlockchain().getHeight() - Constants.MAX_DIVIDEND_PER_BLOCKS) {
                        throw new XinException.NotCurrentlyValidException("Last dividend payment for asset " + Long.toUnsignedString(attachment.getAssetId())
                                + " was less than " + Constants.MAX_DIVIDEND_PER_BLOCKS + " blocks ago at " + lastDividend.getHeight() + ", current height is " + Xin.getBlockchain().getHeight()
                                + ", limit is one dividend per " + Constants.MAX_DIVIDEND_PER_BLOCKS + " blocks");
                    }
                }
            ...
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                Attachment.ColoredCoinsDividendPayment attachment = (Attachment.ColoredCoinsDividendPayment) transaction.getAttachment();
                return Xin.getBlockchain().getHeight() > Constants.DIVIDEND_THROTTLE_BLOCK &&
    ...
    public static abstract class AdvancedPayment extends TransactionType {
        ...
        public final static TransactionType ESCROW_CREATION = new AdvancedPayment() {
            ...
            @Override
            Attachment.AdvancedPaymentEscrowCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.AdvancedPaymentEscrowCreation(buffer, transactionVersion);
            }

            @Override
            Attachment.AdvancedPaymentEscrowCreation parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                Attachment.AdvancedPaymentEscrowCreation attachment = (Attachment.AdvancedPaymentEscrowCreation) transaction.getAttachment();
                Long totalAmountTQT = Math.addExact(attachment.getAmountTQT(), transaction.getFeeTQT());
        ...
        public final static TransactionType ESCROW_SIGN = new AdvancedPayment() {
            ...
            @Override
            Attachment.AdvancedPaymentEscrowSign parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.AdvancedPaymentEscrowSign(buffer, transactionVersion);
            }

            @Override
            Attachment.AdvancedPaymentEscrowSign parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                Attachment.AdvancedPaymentEscrowSign attachment = (Attachment.AdvancedPaymentEscrowSign) transaction.getAttachment();
                if (transaction.getAmountTQT() != 0 || transaction.getFeeTQT() != Constants.FEE_ESCROW_SIGN) {
                    throw new XinException.NotValidException("Escrow signing must have amount 0 and fee of 1");
                }
        ...
        public final static TransactionType ESCROW_RESULT = new AdvancedPayment() {
            ...
            @Override
            Attachment.AdvancedPaymentEscrowResult parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.AdvancedPaymentEscrowResult(buffer, transactionVersion);
            }

            @Override
            Attachment.AdvancedPaymentEscrowResult parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
        ...
        public final static TransactionType SUBSCRIPTION_SUBSCRIBE = new AdvancedPayment() {
            ...
            @Override
            Attachment.AdvancedPaymentSubscriptionSubscribe parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.AdvancedPaymentSubscriptionSubscribe(buffer, transactionVersion);
            }

            @Override
            Attachment.AdvancedPaymentSubscriptionSubscribe parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
        ...
        public final static TransactionType SUBSCRIPTION_CANCEL = new AdvancedPayment() {
            ...
            @Override
            Attachment.AdvancedPaymentSubscriptionCancel parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.AdvancedPaymentSubscriptionCancel(buffer, transactionVersion);
            }

            @Override
            Attachment.AdvancedPaymentSubscriptionCancel parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (subscription == null) {
                    throw new XinException.NotValidException("Subscription cancel must contain current subscription id");
                }
        ...
        public final static TransactionType SUBSCRIPTION_PAYMENT = new AdvancedPayment() {
            ...
            @Override
            Attachment.AdvancedPaymentSubscriptionPayment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.AdvancedPaymentSubscriptionPayment(buffer, transactionVersion);
            }

            @Override
            Attachment.AdvancedPaymentSubscriptionPayment parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
        ...
        public static final TransactionType AUTOMATED_TRANSACTION_CREATION = new AutomatedTransactions() {
            ...
            void doValidateAttachment(Transaction transaction)
                ...
                if (transaction.getSignature() != null && Account.getAccount(transaction.getId()) != null) {
                    ...
                    if (Account.getPublicKey(existingAccount.getId()) != null && !Arrays.equals(Account.getPublicKey(existingAccount.getId()), new byte[32]))
    ...
    public static abstract class AccountControl extends TransactionType {
        ...
        public static final TransactionType EFFECTIVE_BALANCE_LEASING = new AccountControl() {
            ...
            @Override
            Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws XinException.NotValidException {
                return new Attachment.AccountControlEffectiveBalanceLeasing(buffer, transactionVersion);
            }

            @Override
            Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(JSONObject attachmentData) throws XinException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws XinException.ValidationException {
                ...
                if (transaction.getAmountTQT() != 0) {
                    throw new XinException.NotValidException("Transaction amount must be 0 for effective balance leasing");
                }
                if (attachment.getPeriod() < Constants.LEASING_DELAY || attachment.getPeriod() > Constants.EFFECTIVE_LEASING_MAX_BLOCKS) {
                    throw new XinException.NotValidException("Invalid effective balance leasing period: " + attachment.getPeriod());
                }
                ...
                //System.out.println("EffectiveBalanceLeasing getRecipientId : " + senderAccount.getRecipientId());
                ...
                if (senderAccount == null) {
                    throw new XinException.NotValidException("Invalid effective balance leasing: " +
                            "sender account " + Long.toUnsignedString(transaction.getSenderId()) + " not found or no public key published");
                }
                if (senderAccount.getBalanceTQT() < Constants.EFFECTIVE_LEASING_MIN_BALANCE) {
                    throw new XinException.NotValidException("Invalid effective balance leasing: " +
                            "sender account " + Long.toUnsignedString(transaction.getSenderId()) + " must have minimum " +
                            "balance of " + Constants.EFFECTIVE_LEASING_MIN_BALANCE);
                }
                if (recipientAccount.getEffectiveBalanceTQT() > Constants.EFFECTIVE_LEASING_MAX_BALANCETQT) {
                    throw new XinException.NotValidException("Invalid effective balance leasing: " +
...
//                Logger.logErrorMessage(String.format("Leases count for %s is %d when leasing %s",Long.toUnsignedString
//                        (transaction
//                        .getRecipientId()),currentLeaseCount,Long.toUnsignedString(transaction.getSenderId())));
                if (currentLeaseCount > Constants
                        .EFFECTIVE_LEASING_MAX_ACCOUNTS) {
                    throw new XinException.NotValidException("Invalid effective balance leasing: " +
                            "recipient account " + Long.toUnsignedString(transaction.getSenderId()) + " have exceded " +
            ...
            void validateAttachment(Transaction transaction) throws ValidationException {
                ...
                if (votingModel == VotingModel.NONE) {
                    ...
                    if (senderAccount == null || !senderAccount.getControls().contains(ControlType.PHASING_ONLY)) {
                        throw new XinException.NotCurrentlyValidException("Phasing only account control is not currently enabled");
                    }
                } else if (votingModel == VotingModel.TRANSACTION || votingModel == VotingModel.HASH) {
                    throw new XinException.NotValidException("Invalid voting model " + votingModel + " for account control");
                }
                long maxFees = attachment.getMaxFees();
                long maxFeesLimit = (attachment.getPhasingParams().getVoteWeighting().isBalanceIndependent() ? 3 : 22) * Constants.ONE_XIN;
                if (maxFees < 0 || (maxFees > 0 && maxFees < maxFeesLimit) || maxFees > Constants.MAX_BALANCE_TQT) {
                    throw new XinException.NotValidException(String.format("Invalid max fees %f token", ((double) maxFees) / Constants.ONE_XIN));
                }
                ...
                if (minDuration > maxDuration) {
                    throw new XinException.NotValidException(String.format("Min duration %d cannot exceed max duration %d ",
