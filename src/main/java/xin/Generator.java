...
public final class Generator implements Comparable<Generator> {
    ...
    private Generator(String secretPhrase) {
        ...
        if (Xin.getBlockchain().getHeight() >= Constants.LAST_KNOWN_BLOCK) {
    ...
    // Jean-Luc Picard  committed 17a6181
    ...
    //      } else {
    //          effectiveBalance = BigInteger.valueOf(Math.max(account.TKN(height), 0));
    //      }
    //      if (effectiveBalance.signum() == 0) {
    //          hitTime = 0;
    //          hit = BigInteger.ZERO;
