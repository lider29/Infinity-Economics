...
public final class Account {
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = Xin.getBlockchain().getHeight();
        }
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Xin.getBlockchainProcessor().getMinRollbackHeight()) {
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = Xin.getBooleanProperty("xin.enablePublicKeyCache") ?
    ...
    public static int getAccountLeaseCount(long accountId){
        return accountLeaseTable.getCount(new DbClause.LongClause("current_lessee_id",accountId));
    }
    ...
    static {
        ...
        if (publicKeyCache != null) {
            ...
            Xin.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    private long balanceTQT;
    private long unconfirmedBalanceTQT;
    private long forgedBalanceTQT;
    ...
    private Account(ResultSet rs, DbKey dbKey) throws SQLException {
        this.id = rs.getLong("id");
        this.dbKey = dbKey;
        this.balanceTQT = rs.getLong("balance");
        this.unconfirmedBalanceTQT = rs.getLong("unconfirmed_balance");
        this.forgedBalanceTQT = rs.getLong("forged_balance");
    ...
    private void save(Connection con) throws SQLException {
        try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account (id, "
            ...
            int i = 0;
            pstmt.setLong(++i, this.id);
            pstmt.setLong(++i, this.balanceTQT);
            pstmt.setLong(++i, this.unconfirmedBalanceTQT);
            pstmt.setLong(++i, this.forgedBalanceTQT);
    ...
    private void save() {
        if (balanceTQT == 0 && unconfirmedBalanceTQT == 0 && forgedBalanceTQT == 0 && activeLesseeId == 0 && controls.isEmpty()) {
    ...
    public long getBalanceTQT() {
        return balanceTQT;
    }

    public long getUnconfirmedBalanceTQT() {
        return unconfirmedBalanceTQT;
    }

    public long getForgedBalanceTQT() {
        return forgedBalanceTQT;
    }

    public long getEffectiveBalanceTQT(){
        return getEffectiveBalanceTKN()*Constants.ONE_XIN;
    }

    public long getEffectiveBalanceTKN() {
        return getEffectiveBalanceTKN(Xin.getBlockchain().getHeight());
    }

    public long getEffectiveBalanceTKN(int height) {
        ...
        if (height < Constants.EFFECTIVE_LEASING_BLOCK) {

            if (Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS, id) >= 0) {
                return balanceTQT / Constants.ONE_XIN;
            }
            long receivedInLastBlock = 0;
            for (Transaction transaction : Xin.getBlockchain().getBlockAtHeight(height).getTransactions()) {
                if (id == transaction.getRecipientId()) {
                    receivedInLastBlock += transaction.getAmountTQT();
                }
            }

            return (balanceTQT - receivedInLastBlock) / Constants.ONE_XIN;
        }

        Xin.getBlockchain().readLock();
        try {
            long effectiveBalanceTQT = getLessorsGuaranteedBalanceTQT(height);
            if (activeLesseeId == 0) {
                effectiveBalanceTQT += getGuaranteedBalanceTQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, height);
            }

            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceTQT < Constants.MIN_FORGING_BALANCE_TQT) ? 0 : effectiveBalanceTQT / Constants.ONE_XIN;
        } finally {
            Xin.getBlockchain().readUnlock();
        }
    }

    private long getLessorsGuaranteedBalanceTQT(int height) {
        ...
        for (int i = 0; i < lessors.size(); i++) {
            ...
            balances[i] = lessors.get(i).getBalanceTQT();
        }
    ...
    public long getGuaranteedBalanceTQT() {
        return getGuaranteedBalanceTQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, Xin.getBlockchain().getHeight());
    }

    public long getGuaranteedBalanceTQT(final int numberOfConfirmations, final int currentHeight) {
        Xin.getBlockchain().readLock();
        try {
            int height = currentHeight - numberOfConfirmations;
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Xin.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Xin.getBlockchain().getHeight()) {
            ...
            try (Connection con = Db.db.getConnection();
                ...
                try (ResultSet rs = pstmt.executeQuery()) {
                    ...
                    return Math.max(Math.subtractExact(balanceTQT, rs.getLong("additions")), 0);
                }
            } catch (SQLException e) {
                throw new RuntimeException(e.toString(), e);
            }
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = Xin.getBlockchain().getHeight();
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Xin.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= Xin.getBlockchain().getHeight() - 1) {
    ...
    void addToBalanceTQT(LedgerEvent event, long eventId, long amountTQT) {
        addToBalanceTQT(event, eventId, amountTQT, 0);
    }

    void addToBalanceTQT(long amountTQT,long feeTQT){
        if (amountTQT == 0 && feeTQT == 0) {
            return;
        }
        long totalAmountTQT = Math.addExact(amountTQT, feeTQT);
        this.balanceTQT = Math.addExact(this.balanceTQT, totalAmountTQT);
        addToGuaranteedBalanceTQT(totalAmountTQT);
        checkBalance(this.id, this.balanceTQT, this.unconfirmedBalanceTQT);
        save();
        listeners.notify(this, Event.BALANCE);
    }

    void addToBalanceTQT(long amountTQT){
        addToBalanceTQT(amountTQT,0);
    }

    void addToBalanceTQT(LedgerEvent event, long eventId, long amountTQT, long feeTQT) {
        if (amountTQT == 0 && feeTQT == 0) {
            return;
        }
        long totalAmountTQT = Math.addExact(amountTQT, feeTQT);
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeTQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.XIN_BALANCE, null, feeTQT, this.balanceTQT - amountTQT));
            }
            if (amountTQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.XIN_BALANCE, null, amountTQT, this.balanceTQT));
            }
        }
    }

    void addToUnconfirmedBalanceTQT(LedgerEvent event, long eventId, long amountTQT) {
        addToUnconfirmedBalanceTQT(event, eventId, amountTQT, 0);
    }

    void addToUnconfirmedBalanceTQT(long amountTQT){
        addToUnconfirmedBalanceTQT(amountTQT,0);
    }
    void addToUnconfirmedBalanceTQT(long amountTQT,long feeTQT){
        if (amountTQT == 0 && feeTQT == 0) {
            return;
        }
        long totalAmountTQT = Math.addExact(amountTQT, feeTQT);
        this.unconfirmedBalanceTQT = Math.addExact(this.unconfirmedBalanceTQT, totalAmountTQT);
        checkBalance(this.id, this.balanceTQT, this.unconfirmedBalanceTQT);
        save();
        listeners.notify(this, Event.UNCONFIRMED_BALANCE);
    }

    void addToUnconfirmedBalanceTQT(LedgerEvent event, long eventId, long amountTQT, long feeTQT) {
        if (amountTQT == 0 && feeTQT == 0) {
            return;
        }
        long totalAmountTQT = Math.addExact(amountTQT, feeTQT);
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeTQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_XIN_BALANCE, null, feeTQT, this.unconfirmedBalanceTQT - amountTQT));
            }
            if (amountTQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_XIN_BALANCE, null, amountTQT, this.unconfirmedBalanceTQT));
            }
        }
    }

    void addToBalanceAndUnconfirmedBalanceTQT(LedgerEvent event, long eventId, long amountTQT) {
        addToBalanceAndUnconfirmedBalanceTQT(event, eventId, amountTQT, 0);
    }

    void addToBalanceAndUnconfirmedBalanceTQT(long amountTQT){
        addToBalanceAndUnconfirmedBalanceTQT(amountTQT,0);
    }
    void addToBalanceAndUnconfirmedBalanceTQT(long amountTQT,long feeTQT){
        if (amountTQT == 0 && feeTQT == 0) {
            return;
        }
        long totalAmountTQT = Math.addExact(amountTQT, feeTQT);
    ...
    void addToBalanceAndUnconfirmedBalanceTQT(LedgerEvent event, long eventId, long amountTQT, long feeTQT) {
        if (amountTQT == 0 && feeTQT == 0) {
            return;
        }
        long totalAmountTQT = Math.addExact(amountTQT, feeTQT);
    ...
    void addToForgedBalanceTQT(long amountTQT) {
        if (amountTQT == 0) {
            return;
        }
        this.forgedBalanceTQT = Math.addExact(this.forgedBalanceTQT, amountTQT);
        save();
    }
    ...
    private void addToGuaranteedBalanceTQT(long amountTQT) {
        if (amountTQT <= 0) {
            return;
        }
        int blockchainHeight = Xin.getBlockchain().getHeight();
    ...
    void payDividends(final long transactionId, Attachment.ColoredCoinsDividendPayment attachment) {
        ...
        final long amountTQTPerQNT = attachment.getAmountTQTPerQNT();
        long numAccounts = 0;
        for (final AccountAsset accountAsset : accountAssets) {
            if (accountAsset.getAccountId() != this.id && accountAsset.getQuantityQNT() != 0) {
                long dividend = Math.multiplyExact(accountAsset.getQuantityQNT(), amountTQTPerQNT);
                Account.getAccount(accountAsset.getAccountId())
                        .addToBalanceAndUnconfirmedBalanceTQT(LedgerEvent.ASSET_DIVIDEND_PAYMENT, transactionId, dividend);
                totalDividend += dividend;
                numAccounts += 1;
            }
        }
        this.addToBalanceTQT(LedgerEvent.ASSET_DIVIDEND_PAYMENT, transactionId, -totalDividend);
