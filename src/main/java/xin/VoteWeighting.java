...
public final class VoteWeighting {
    ...
    public enum VotingModel {
        ...
        TQT(1) {
            @Override
            public final long calcWeight(VoteWeighting voteWeighting, long voterId, int height) {
                long tqtBalance = Account.getAccount(voterId, height).getBalanceTQT();
                return tqtBalance >= voteWeighting.minBalance ? tqtBalance : 0;
            }

            @Override
            public final MinBalanceModel getMinBalanceModel() {
                return MinBalanceModel.TQT;
            }
        },
        ASSET(2) {
            @Override
    ...
    public enum MinBalanceModel {
        ...
        TQT(1) {
            @Override
            public final long getBalance(VoteWeighting voteWeighting, long voterId, int height) {
                return Account.getAccount(voterId, height).getBalanceTQT();
            }
        },
        ASSET(2) {
            @Override
    ...
    private final long holdingId; //either asset id or coin id
    ...
    public void validate() throws XinException.ValidationException {
        if (votingModel == null) {
            throw new XinException.NotValidException("Invalid voting model");
        }
        if (minBalanceModel == null) {
            throw new XinException.NotValidException("Invalid min balance model");
        }
        if ((votingModel == VotingModel.ASSET || votingModel == VotingModel.CURRENCY) && holdingId == 0) {
            throw new XinException.NotValidException("No holdingId provided");
        }
        if (votingModel == VotingModel.CURRENCY && Currency.getCurrency(holdingId) == null) {
            throw new XinException.NotCurrentlyValidException("Currency " + Long.toUnsignedString(holdingId) + " not found");
        }
        if (votingModel == VotingModel.ASSET && Asset.getAsset(holdingId) == null) {
            throw new XinException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(holdingId) + " not found");
        }
        if (minBalance < 0) {
            throw new XinException.NotValidException("Invalid minBalance " + minBalance);
        }
        if (minBalance > 0) {
            if (minBalanceModel == MinBalanceModel.NONE) {
                throw new XinException.NotValidException("Invalid min balance model " + minBalanceModel);
            }
            if (votingModel.getMinBalanceModel() != MinBalanceModel.NONE && votingModel.getMinBalanceModel() != minBalanceModel) {
                throw new XinException.NotValidException("Invalid min balance model: " + minBalanceModel + " for voting model " + votingModel);
            }
            ...
            if (minBalanceModel == MinBalanceModel.ASSET && Asset.getAsset(holdingId) == null) {
                throw new XinException.NotCurrentlyValidException("Invalid min balance asset: " + Long.toUnsignedString(holdingId));
            }
            if (minBalanceModel == MinBalanceModel.CURRENCY && Currency.getCurrency(holdingId) == null) {
                throw new XinException.NotCurrentlyValidException("Invalid min balance currency: " + Long.toUnsignedString(holdingId));
            }
        }
        if (minBalance == 0 && votingModel == VotingModel.ACCOUNT && holdingId != 0) {
            throw new XinException.NotValidException("HoldingId cannot be used in by account voting with no min balance");
        }
        if ((votingModel == VotingModel.TQT || minBalanceModel == MinBalanceModel.TQT) && holdingId != 0) {
            throw new XinException.NotValidException("HoldingId cannot be used in by balance voting or with min balance in TQT");
        }
        if ((!votingModel.acceptsVotes() || votingModel == VotingModel.HASH) && (holdingId != 0 || minBalance != 0 || minBalanceModel != MinBalanceModel.NONE)) {
            throw new XinException.NotValidException("With VotingModel " + votingModel + " no holdingId, minBalance, or minBalanceModel should be specified");
        }
    }
