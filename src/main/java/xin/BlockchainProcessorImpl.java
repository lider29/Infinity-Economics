...
final class BlockchainProcessorImpl implements BlockchainProcessor {
    ...
    private static final byte[] CHECKSUM_TQT_BLOCK = null;
    private static final byte[] CHECKSUM_MONETARY_SYSTEM_BLOCK = null;
    private static final byte[] CHECKSUM_PHASING_BLOCK = null;
    private static final byte[] CHECKSUM_16 = null;
    private static final byte[] CHECKSUM_17 = null;
    private static final byte[] CHECKSUM_18 = null;
    ...
    private final boolean trimDerivedTables = Xin.getBooleanProperty("xin.trimDerivedTables");

    private final int defaultNumberOfForkConfirmations = Xin.getIntProperty("xin.numberOfForkConfirmations");
    ...
    private final Runnable getMoreBlocksThread = new Runnable() {
        ...
        public void run() {
            try {
                ...
                Logger.logErrorMessage("CRITICAL ERROR. PLEASE REPORT.\n" + t.toString(), t);
                System.exit(1);
            }
        }
        ...
        private void downloadPeer() throws InterruptedException {
            ...
            } catch (XinException.StopException e) {
    ...
    private static class GetNextBlocks implements Callable<List<BlockImpl>> {
        ...
        public List<BlockImpl> call() {
            ...
            } catch (RuntimeException | XinException.NotValidException e) {
        ...
        public void run() {
            ...
            try {
                ...
                while (!processing.isEmpty()) {
                    ...
                    List<Transaction> processed = Xin.getTransactionProcessor().restorePrunableData(transactions);
                    //
            ...
            } catch (XinException.ValidationException e) {
    ...
    private final Listener<Block> checksumListener = block -> {
      ...
      // start checksum with escrow, subscription and AT blocks
      // generate first from block 100000

      // if (block.getHeight() == Constants.CHECKSUM_BLOCK_1
      //         && !verifyChecksum(CHECKSUM_BLOCK_1, 0, Constants.CHECKSUM_BLOCK_1)) {
      //     popOffTo(0);
      // }
    ...
    private BlockchainProcessorImpl() {
        final int trimFrequency = Xin.getIntProperty("xin.trimFrequency");
        ...
        ThreadPool.runBeforeStart(() -> {
            ...
            if (addGenesisBlock()) {
                scan(0, false);
            } else if (Xin.getBooleanProperty("xin.forceScan")) {
                scan(0, Xin.getBooleanProperty("xin.forceValidate"));
            } else {
                boolean rescan;
                boolean validate;
                int height;
    ...
    public void registerDerivedTable(DerivedDbTable table) {
        if (alreadyInitialized) {
            throw new IllegalStateException("Too late to register table " + table + ", must have done it in Xin.Init");
        }
    ...
    @Override
    public void processPeerBlock(JSONObject request) throws XinException {
    ...
    public Transaction restorePrunedTransaction(long transactionId) {
        ...
        // Jean-Luc Picard  committed 073503b
        // if (transaction == null) {
        ...
        try {
            ...
            for (int i = 0; i < Genesis.GENESIS_RECIPIENTS.length; i++) {
                TransactionImpl transaction = new TransactionImpl.BuilderImpl((byte) 0, Genesis.CREATOR_PUBLIC_KEY,
                        Genesis.GENESIS_AMOUNTS[i] * Constants.ONE_XIN, 0, (short) 0,
            ...
            BlockImpl genesisBlock = new BlockImpl(-1, 0, 0, Constants.MAX_BALANCE_TQT, 0, transactions.size() * 128, digest.digest(),
                    Genesis.CREATOR_PUBLIC_KEY, new byte[64], Genesis.GENESIS_BLOCK_SIGNATURE, null, transactions,byteATs);
    ...
    private void validateTransactions(BlockImpl block, BlockImpl previousLastBlock, int curTime, Map<TransactionType, Map<String, Integer>> duplicates,
        ...
        for (TransactionImpl transaction : block.getTransactions()) {
            ...
            calculatedTotalAmount += transaction.getAmountTQT();
            calculatedTotalFee += transaction.getFeeTQT();
            payloadLength += transaction.getFullSize();
            digest.update(transaction.bytes());
        }
        if (calculatedTotalAmount != block.getTotalAmountTQT() || calculatedTotalFee != block.getTotalFeeTQT()) {
    ...
    private void accept(BlockImpl block, List<TransactionImpl> validPhasedTransactions, List<TransactionImpl> invalidPhasedTransactions,
        ...
        try {
            ...
            //TODO:Some of these could be in validate()
            try {
                atBlock = AT_Controller.validateATs( block.getBlockATs() , Xin.getBlockchain().getHeight());
            } catch (NoSuchAlgorithmException e) {
                //should never reach that point
                throw new BlockNotAcceptedException( "md5 does not exist",block );
            } catch (AT_Exception e) {
                throw new BlockNotAcceptedException("ats are not matching at block height " + Xin.getBlockchain()
                        .getHeight(),block );
            }

            if(Subscription.isEnabled()) {
                Subscription.applyUnconfirmed(block.getTimestamp());
            }
            ...
            int fromTimestamp = Xin.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME;
            for (TransactionImpl transaction : block.getTransactions()) {
                try {
                    transaction.apply();
    ...
    private int getBlockVersion(int previousBlockHeight) {
       return previousBlockHeight < Constants.TRANSPARENT_FORGING_BLOCK ? 1
               : previousBlockHeight < Constants.TQT_BLOCK ? 2
               : 3;
    }

    private int getTransactionVersion(int previousBlockHeight) {
        return previousBlockHeight < 1 ? 0 : 1;
    }
        ...
        while (payloadLength <= Constants.MAX_PAYLOAD_LENGTH && sortedTransactions.size() <= Constants.MAX_NUMBER_OF_TRANSACTIONS) {
            ...
            for (UnconfirmedTransaction unconfirmedTransaction : orderedUnconfirmedTransactions) {
                ...
                try {
                    ...
                    //Uncomment below line to see log of failed unconfirmed transactions
    ...
    void generateBlock(String secretPhrase, int blockTimestamp) throws BlockNotAcceptedException {
        ...
        if (blockchain.getHeight() >= Constants.PHASING_BLOCK) {
            try (DbIterator<TransactionImpl> phasedTransactions = PhasingPoll.getFinishingTransactions(blockchain.getHeight() + 1)) {
                for (TransactionImpl phasedTransaction : phasedTransactions) {
                    ...
                    } catch (XinException.ValidationException ignore) {
                    }
                }
            }
        }
        ...
        for (UnconfirmedTransaction unconfirmedTransaction : sortedTransactions) {
            ...
            digest.update(transaction.bytes());
            totalAmountTQT += transaction.getAmountTQT();
            totalFeeTQT += transaction.getFeeTQT();
            payloadLength += transaction.getFullSize();
        }
        ...
        if(Subscription.isEnabled()) {
            synchronized(blockchain) {
                ...
                try {
                    ...
                    //totalFeeTQT +=Subscription.calculateFees(blockTimestamp);
                    Subscription.calculateFees(blockTimestamp);
                }
                finally {
                    Db.db.rollbackTransaction();
                    Db.db.endTransaction();
        ...
        if (byteATs!=null)
        {
            ...
            atBlock.getTotalFees();
            atBlock.getTotalAmount();
        ...
        BlockImpl block = new BlockImpl(getBlockVersion(previousBlock.getHeight()), blockTimestamp, previousBlock.getId(), totalAmountTQT, totalFeeTQT, payloadLength,
                payloadHash, publicKey, generationSignature, previousBlockHash, blockTransactions, byteATs,
                secretPhrase);
        ...
        try {
            ...
            Logger.logDebugMessage("Account " + Long.toUnsignedString(block.getGeneratorId()) + " generated block " + block.getStringId()
                    + " at height " + block.getHeight() + " timestamp " + block.getTimestamp() + " fee " + ((float) block.getTotalFeeTQT()) / Constants.ONE_XIN);
        } catch (TransactionNotAcceptedException e) {
            Logger.logDebugMessage("Generate block failed: " + e.getMessage(),e);
            TransactionProcessorImpl.getInstance().processWaitingTransactions();
    ...
    boolean hasAllReferencedTransactions(TransactionImpl transaction, int timestamp, int count) {
        ...
        //TODO:This could be a potential issue
        if(!Subscription.isEnabled()) {
            if(referencedTransaction!= null && referencedTransaction.getSignature() == null) {
                    ...
                    while (rs.next()) {
                        try {
                            ...
                            if (currentBlock.getId() != currentBlockId || currentBlock.getHeight() > blockchain.getHeight() + 1) {
                                throw new XinException.NotValidException("Database blocks in the wrong order!");
                            }
                            Map<TransactionType, Map<String, Integer>> duplicates = new HashMap<>();
                            List<TransactionImpl> validPhasedTransactions = new ArrayList<>();
                            List<TransactionImpl> invalidPhasedTransactions = new ArrayList<>();
                            ...
                            if (validate && currentBlockId != Genesis.GENESIS_BLOCK_ID) {
                                ...
                                if (!Arrays.equals(blockBytes, BlockImpl.parseBlock(blockJSON).bytes())) {
                                    throw new XinException.NotValidException("Block JSON cannot be parsed back to the same block");
                                }
                                ...
                                for (TransactionImpl transaction : currentBlock.getTransactions()) {
                                    ...
                                    if (currentBlock.getHeight() > Constants.TQT_BLOCK
                                            && !Arrays.equals(transactionBytes, TransactionImpl.newTransactionBuilder(transactionBytes).build().bytes())) {
                                        throw new XinException.NotValidException("Transaction bytes cannot be parsed back to the same transaction: "
                                    ...
                                    if (!Arrays.equals(transactionBytes, TransactionImpl.newTransactionBuilder(transactionJSON).build().bytes())) {
                                        throw new XinException.NotValidException("Transaction JSON cannot be parsed back to the same transaction: "
                                                + transaction.getJSONObject().toJSONString());
                                    }
                                }
                            }
