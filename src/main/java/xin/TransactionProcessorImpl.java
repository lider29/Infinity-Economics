...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = Xin.getBooleanProperty("xin.enableTransactionRebroadcasting");

    private static final boolean testUnconfirmedTransactions = Xin.getBooleanProperty("xin.testUnconfirmedTransactions");


    private static final int maxUnconfirmedTransactions;

    static {
        int n = Xin.getIntProperty("xin.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (Xin.getBlockchainProcessor().isDownloading() && !testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, Xin.getEpochTime()), 0, -1, "")) {
    ...
    private final Runnable processTransactionsThread = () -> {
        ...
        try {
            try {
                ...
                } catch (XinException.ValidationException | RuntimeException e) {
                    peer.blacklist(e);
                }
            } catch (Exception e) {
    ...
    @Override
    public void broadcast(Transaction transaction) throws XinException.ValidationException {
        BlockchainImpl.getInstance().writeLock();
        try {
            ...
            //Uncomment below line to see why a transaction has failed
    ...
    @Override
    public void processPeerTransactions(JSONObject request) throws XinException.ValidationException {
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = Xin.getEpochTime();
                ...
                while (iterator.hasNext()) {
                    ...
                    try {
                        ...
                        // Jean-Luc Picard  committed e76e888
                        // unconfirmedTransaction.validate();

                        processTransaction(unconfirmedTransaction);
                        iterator.remove();
                        addedUnconfirmedTransactions.add(unconfirmedTransaction.getTransaction());
                    } catch (XinException.ExistingTransactionException e) {
                        iterator.remove();
                    } catch (XinException.NotCurrentlyValidException e) {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws XinException.NotValidException {
        if (Xin.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
            return;
        }
        ...
        for (Object transactionData : transactionsData) {
            ...
            } catch (XinException.NotCurrentlyValidException ignore) {
                // Logger.logDebugMessage(ignore.getMessage(),ignore);
            } catch (XinException.ValidationException | RuntimeException e) {
        ...
        if (!exceptions.isEmpty()) {
            throw new XinException.NotValidException("Peer sends invalid transactions: " + exceptions.toString());
        }
    }

    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws XinException.ValidationException {
        TransactionImpl transaction = unconfirmedTransaction.getTransaction();
        int curTime = Xin.getEpochTime();
        if (transaction.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT || transaction.getExpiration() < curTime) {
            throw new XinException.NotCurrentlyValidException("Invalid transaction timestamp");
        }
        if (transaction.getVersion() < 1) {
            throw new XinException.NotValidException("Invalid transaction version");
        }
        if (transaction.getId() == 0L) {
            throw new XinException.NotValidException("Invalid transaction id 0");
        }
        ...
        try {
            try {
                Db.db.beginTransaction();
                if (Xin.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
                    throw new XinException.NotCurrentlyValidException("Blockchain not ready to accept transactions");
                }

                if (getUnconfirmedTransaction(transaction.getDbKey()) != null || TransactionDb.hasTransaction(transaction.getId())) {
                    throw new XinException.ExistingTransactionException("Transaction already processed");
                }

                if (!transaction.verifySignature()) {
                    if (Account.getAccount(transaction.getSenderId()) != null) {
                        throw new XinException.NotValidException("Transaction signature verification failed");
                    } else {
                        throw new XinException.NotCurrentlyValidException("Unknown transaction sender");
                    }
                }

                if (!transaction.applyUnconfirmed()) {
                    throw new XinException.InsufficientBalanceException("Insufficient balance");
                }

                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    throw new XinException.NotCurrentlyValidException("Duplicate unconfirmed transaction");
                }
     ...
     * @throws XinException.NotValidException Transaction is not valid
     */
    @Override
    public List<Transaction> restorePrunableData(JSONArray transactions) throws XinException.NotValidException {
