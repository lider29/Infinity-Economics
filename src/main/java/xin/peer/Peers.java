...
public final class Peers {
    ...
    static final boolean useProxy = System.getProperty("socksProxyHost") != null || System.getProperty("api.proxyHost") != null;
    static final boolean isGzipEnabled;

    private static final int DEFAULT_PEER_PORT = Constants.DEFAULT_PEER_PORT;
    ...
    static final boolean hideErrorDetails = Xin.getBooleanProperty("xin.hideErrorDetails");
    ...
    static {
        ...
        String platform = Xin.getStringProperty("xin.myPlatform", System.getProperty("os.name") + " " + System.getProperty("os.arch"));
        if (platform.length() > MAX_PLATFORM_LENGTH) {
            platform = platform.substring(0, MAX_PLATFORM_LENGTH);
        }
        myPlatform = platform;
        myAddress = Convert.emptyToNull(Xin.getStringProperty("xin.myAddress", "").trim());
        ...
        myPeerServerPort = Xin.getIntProperty("xin.peerServerPort");


        shareMyAddress = Xin.getBooleanProperty("xin.shareMyAddress") && !Constants.isOffline;
        enablePeerUPnP = Xin.getBooleanProperty("xin.enablePeerUPnP");


        // myHallmark = Convert.emptyToNull(Xin.getStringProperty("xin.myHallmark", "").trim());

        myHallmark = Xin.getStringProperty("xin.myHallmark");
        ...
        json.put("application", Xin.APPLICATION);
        json.put("version", Xin.VERSION);
        ...
        final List<String> defaultPeers = Xin.getStringListProperty("xin.defaultPeers");
        wellKnownPeers = Collections.unmodifiableList(Xin.getStringListProperty("xin.wellKnownPeers"));

        List<String> knownBlacklistedPeersList = Xin.getStringListProperty("xin.knownBlacklistedPeers");
        ...
        maxNumberOfInboundConnections = Xin.getIntProperty("xin.maxNumberOfInboundConnections");
        maxNumberOfOutboundConnections = Xin.getIntProperty("xin.maxNumberOfOutboundConnections");
        maxNumberOfConnectedPublicPeers = Math.min(Xin.getIntProperty("xin.maxNumberOfConnectedPublicPeers"),
                maxNumberOfOutboundConnections);
        maxNumberOfKnownPeers = Xin.getIntProperty("xin.maxNumberOfKnownPeers");
        minNumberOfKnownPeers = Xin.getIntProperty("xin.minNumberOfKnownPeers");
        connectTimeout = Xin.getIntProperty("xin.connectTimeout");
        readTimeout = Xin.getIntProperty("xin.readTimeout");
        enableHallmarkProtection = Xin.getBooleanProperty("xin.enableHallmarkProtection");
        pushThreshold = Xin.getIntProperty("xin.pushThreshold");
        pullThreshold = Xin.getIntProperty("xin.pullThreshold");
        useWebSockets = Xin.getBooleanProperty("xin.useWebSockets");
        webSocketIdleTimeout = Xin.getIntProperty("xin.webSocketIdleTimeout");
        isGzipEnabled = Xin.getBooleanProperty("xin.enablePeerServerGZIPFilter");
        blacklistingPeriod = Xin.getIntProperty("xin.blacklistingPeriod") / 1000;
        communicationLoggingMask = Xin.getIntProperty("xin.communicationLoggingMask");
        sendToPeersLimit = Xin.getIntProperty("xin.sendToPeersLimit");
        usePeersDb = Xin.getBooleanProperty("xin.usePeersDb") && !Constants.isOffline;
        savePeers = usePeersDb && Xin.getBooleanProperty("xin.savePeers");
        getMorePeers = Xin.getBooleanProperty("xin.getMorePeers");
        cjdnsOnly = Xin.getBooleanProperty("xin.cjdnsOnly");
        ignorePeerAnnouncedAddress = Xin.getBooleanProperty("xin.ignorePeerAnnouncedAddress");
        ...
        if (!Constants.isOffline) {
            ThreadPool.runBeforeStart(new Runnable() {
                ...
                @Override
                public void run() {
                    final int now = Xin.getEpochTime();
                        ...
                        if (savePeers) {
                            ...
                            dbPeers.forEach(entry -> {
                                if (!entries.add(entry)) {
                                    // Database entries override entries from node.properties
    ...
    private static class Init {
        ...
        static {
            if (Peers.shareMyAddress) {
                ...
                final String host = Xin.getStringProperty("xin.peerServerHost");
                connector.setHost(host);
                connector.setIdleTimeout(Xin.getIntProperty("xin.peerServerIdleTimeout"));
                ...
                if (Xin.getBooleanProperty("xin.enablePeerServerDoSFilter")) {
                    FilterHolder dosFilterHolder = ctxHandler.addFilter(DoSFilter.class, "/*",
                            EnumSet.of(DispatcherType.REQUEST));
                    dosFilterHolder.setInitParameter("maxRequestsPerSec", Xin.getStringProperty("xin.peerServerDoSFilter.maxRequestsPerSec"));
                    dosFilterHolder.setInitParameter("delayMs", Xin.getStringProperty("xin.peerServerDoSFilter.delayMs"));
                    dosFilterHolder.setInitParameter("maxRequestMs", Xin.getStringProperty("xin.peerServerDoSFilter.maxRequestMs"));
