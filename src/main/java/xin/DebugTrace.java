...
public final class DebugTrace {
    ...
    static final String QUOTE = Xin.getStringProperty("xin.debugTraceQuote", "\"");
    static final String SEPARATOR = Xin.getStringProperty("xin.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Xin.getBooleanProperty("xin.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Xin.getStringListProperty("xin.debugTraceAccounts");
        String logName = Xin.getStringProperty("xin.debugTraceLog");
        ...
        Xin.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Xin.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Xin.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Xin.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void trace(Block block) {
        for (Transaction transaction : block.getTransactions()) {
            ...
            if (transaction.getAmountTQT() > 0 && recipientId == 0) {
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            map.put("transaction fee", String.valueOf(-Constants.SHUFFLING_DEPOSIT_TQT));
            map.put("event", "shuffling blame");
            log(map);
            long fee = Constants.SHUFFLING_DEPOSIT_TQT / 4;
            ...
            fee = Constants.SHUFFLING_DEPOSIT_TQT - 3 * fee;
            long generatorId = Xin.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String, String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String, String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("lessor guaranteed balance", String.valueOf(account.getGuaranteedBalanceTQT()));
        map.put("lessee", Long.toUnsignedString(lesseeId));
        map.put("timestamp", String.valueOf(Xin.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Xin.getBlockchain().getHeight()));
    ...
    private void crowdfunding(Currency currency) {
        ...
        try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
            for (CurrencyFounder founder : founders) {
                totalAmountPerUnit += founder.getAmountPerUnitTQT();
                currencyFounders.add(founder);
            }
        }
        for (CurrencyFounder founder : currencyFounders) {
            long units = Math.multiplyExact(remainingSupply, founder.getAmountPerUnitTQT()) / totalAmountPerUnit;
        ...
        if (!currency.is(CurrencyType.CLAIMABLE)) {
            map.put("currency cost", String.valueOf(Math.multiplyExact(currency.getReserveSupply(), currency.getCurrentReservePerUnitTQT())));
        }
    ...
    private void undoCrowdfunding(Currency currency) {
        try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
            for (CurrencyFounder founder : founders) {
                Map<String, String> founderMap = getValues(founder.getAccountId(), false);
                founderMap.put("currency", Long.toUnsignedString(currency.getId()));
                founderMap.put("currency cost", String.valueOf(Math.multiplyExact(currency.getReserveSupply(), founder.getAmountPerUnitTQT())));
    ...
    private void delete(Currency currency) {
        ...
        if (currency.is(CurrencyType.RESERVABLE)) {
            if (currency.is(CurrencyType.CLAIMABLE) && currency.isActive()) {
                map.put("currency cost", String.valueOf(Math.multiplyExact(units, currency.getCurrentReservePerUnitTQT())));
            }
            if (!currency.isActive()) {
                try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
                    for (CurrencyFounder founder : founders) {
    ...
    private Map<String, String> getValues(long accountId, boolean unconfirmed) {
        ...
        Account account = Account.getAccount(accountId);
        map.put("balance", String.valueOf(account != null ? account.getBalanceTQT() : 0));
        map.put("unconfirmed balance", String.valueOf(account != null ? account.getUnconfirmedBalanceTQT() : 0));
        map.put("timestamp", String.valueOf(Xin.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Xin.getBlockchain().getHeight()));
    ...
    private Map<String, String> getValues(long accountId, Trade trade, boolean isAsk) {
        ...
        map.put("trade price", String.valueOf(trade.getPriceTQT()));
        long tradeCost = Math.multiplyExact(trade.getQuantityQNT(), trade.getPriceTQT());
    ...
    private Map<String, String> getValues(long accountId, Shuffling shuffling, boolean isRecipient) {
        ...
        String deposit = String.valueOf(isRecipient ? Constants.SHUFFLING_DEPOSIT_TQT : -Constants.SHUFFLING_DEPOSIT_TQT);
        if (shuffling.getHoldingType() == HoldingType.XIN) {
    ...
    private Map<String, String> getValues(long accountId, Transaction transaction, boolean isRecipient, boolean logFee, boolean logAmount) {
        ...
        long fee = transaction.getFeeTQT();
        if (isRecipient) {
    ...
    private Map<String, String> getValues(long accountId, Block block) {
        ...
        if (block.getHeight() > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                if (include(previousGeneratorId)) {
                    ...
                    map.put("effective balance", String.valueOf(Account.getAccount(previousGeneratorId).getEffectiveBalanceTKN()));
        ...
        Map<String, String> map = getValues(accountId, false);
        map.put("effective balance", String.valueOf(Account.getAccount(accountId).getEffectiveBalanceTKN()));
    ...
    private Map<String, String> getValues(long accountId, Transaction transaction, Attachment attachment, boolean isRecipient) {
        ...
        if (attachment instanceof Attachment.ColoredCoinsOrderPlacement) {
            ...
            map.put("order price", String.valueOf(orderPlacement.getPriceTQT()));
            long quantity = orderPlacement.getQuantityQNT();
            if (isAsk) {
                quantity = -quantity;
            }
            map.put("order quantity", String.valueOf(quantity));
            BigInteger orderCost = BigInteger.valueOf(orderPlacement.getPriceTQT()).multiply(BigInteger.valueOf(orderPlacement.getQuantityQNT()));
            if (!isAsk) {
            ...
            map.put("buy rate", String.valueOf(publishOffer.getBuyRateTQT()));
            map.put("sell rate", String.valueOf(publishOffer.getSellRateTQT()));
            ...
            BigInteger buyCost = BigInteger.valueOf(publishOffer.getBuyRateTQT()).multiply(BigInteger.valueOf(buyUnits));
            map.put("buy cost", buyCost.toString());
            BigInteger sellCost = BigInteger.valueOf(publishOffer.getSellRateTQT()).multiply(BigInteger.valueOf(sellUnits));
            ...
            map.put("currency cost", String.valueOf(Math.multiplyExact(claim.getUnits(), currency.getCurrentReservePerUnitTQT())));
            ...
            map.put("currency cost", String.valueOf(-Math.multiplyExact(reserveIncrease.getAmountPerUnitTQT(), currency.getReserveSupply())));
            ...
            try (DbIterator<Account.AccountAsset> iterator = Account.getAssetAccounts(dividendPayment.getAssetId(), dividendPayment.getHeight(), 0, -1)) {
                while (iterator.hasNext()) {
                    ...
                    if (accountAsset.getAccountId() != accountId && accountAsset.getQuantityQNT() != 0) {
                        long dividend = Math.multiplyExact(accountAsset.getQuantityQNT(), dividendPayment.getAmountTQTPerQNT());
